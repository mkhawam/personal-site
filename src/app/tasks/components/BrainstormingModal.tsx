...
import { 
  Sparkles, Check, Paperclip, Globe, X, ArrowUpCircle, FileText
} from 'lucide-react';
...
export default function BrainstormingModal({ onAddTask, onClose }: BrainstormingModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newHistory = [...chatMessages, { role: 'user', content: finalContent } as ChatMessage]; 
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar flex flex-col">
                ...
                <div className="flex flex-col space-y-4 pb-2">
                    ...
                    {chatMessages.map((msg, idx) => (
                        <div key={idx} className={clsx("flex flex-col max-w-[90%] md:max-w-[75%]", msg.role === 'user' ? "self-end items-end" : "self-start items-start")}>
                            <div className={clsx(
                                        ...
                                        code: ({node, inline, className, children, ...props}: any) => {
                                            ...
                                            return !inline ? (
                                                <div className="relative group/code my-2 max-w-full">
                                                    <code className={clsx("block bg-black/30 p-3 rounded-lg text-sm font-mono overflow-x-auto whitespace-pre-wrap md:whitespace-pre", className)} {...props}>
                                                        {children}
                                                    </code>
                                                </div>
                                            ) : (
            ...
            >
                ...
                <div className="flex items-end gap-2">
                    ...
                    <div className="flex pb-2 pl-2 gap-1">
                        <label className="p-2 hover:bg-white/10 rounded-full text-zinc-500 hover:text-zinc-300 cursor-pointer transition-colors" title="Attach File">
                            <Paperclip size={18} />
                            <input 
                                type="file" 
                                className="hidden" 
                        ...
                        <button 
                            ...
                            className={clsx("p-2 rounded-full transition-colors", showUrlInput || contextUrl ? "text-blue-400 bg-blue-500/10" : "text-zinc-500 hover:text-zinc-300 hover:bg-white/10")}
